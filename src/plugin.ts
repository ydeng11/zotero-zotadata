...
import type { AddonData, MetadataResult, PluginConfig } from '@/shared/core/types';
...
export class ZotadataPlugin {
  ...
  private async registerMenusWithMenuAPI(): Promise<void> {
    ...
    const menus: Zotero.MenuManager.MenuData[] = LIBRARY_ITEM_MENU_L10N_IDS.map(
      (l10nID, i) => ({
        menuType: 'menuitem',
        l10nID,
        onShowing: (_e, ctx) => {
          ...
          const label = LIBRARY_ITEM_MENU_LABELS[i];
          c.menuElem?.setAttribute('label', label);
          const items = c.items;
          let enabled = true;
    ...
    if (registered !== false) {
      ...
      this.log('MenuManager.registerMenu returned false; menus may be unavailable');
    }
  }
  ...
  private async handleFetchMetadata(): Promise<void> {
    return this.errorManager.wrapAsync(
      async () => {
        ...
        const results = await this.getMetadataFetcher().fetchMetadataForSelectedItems();
        this.showMessage(this.formatMetadataFetchSummary(results));
      },
      ErrorType.ZOTERO_ERROR,
  ...
  private formatMetadataFetchSummary(results: MetadataResult[]): string {
    ...
    if (results.length === 1) {
      const r = results[0];
      if (r.success) {
        const lines = [`Updated from ${r.source}.`];
    ...
    const lines = [
      `Metadata fetch finished: ${ok} of ${results.length} item(s) updated.`,
    ];
  ...
  private async handleProcessArxiv(): Promise<void> {
    return this.errorManager.wrapAsync(
      async () => {
        ...
        this.showBriefSuccess(summary, 'arXiv');
      },
      ErrorType.ZOTERO_ERROR,
  ...
  private async handleFindFiles(): Promise<void> {
    return this.errorManager.wrapAsync(
      async () => {
        ...
        this.showBriefSuccess(summary, 'Find Files');
      },
      ErrorType.ZOTERO_ERROR,
  ...
  /**
   * Non-blocking completion notice (avoids modal alert() which reads like a script error).
   */
  private showBriefSuccess(message: string, headline: string): void {
    try {
      const win = new Zotero.ProgressWindow({ closeOnClick: true });
      win.changeHeadline(`Zotadata — ${headline}`);
      for (const line of message.split('\n')) {
        if (line.trim()) {
          win.addDescription(line.trim());
        }
      }
      win.show();
      win.startCloseTimer(6000);
    } catch (error) {
