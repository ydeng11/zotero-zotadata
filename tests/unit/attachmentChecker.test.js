  ...
  describe("AttachmentChecker", () => {
    ...
    beforeEach(() => {
      ...
      global.OS.File.exists = async (path) => {
        ...
        console.log(
          `[Test Suite] OS.File.exists: Path ${path} -> default false`,
        );
        return false;
      };
    ...
    it('should return "valid" for an item with an existing attachment', async () => {
      const mockItem = {
        ...
        getAttachmentsObjects: async () => [
          { path: MOCK_VALID_PATH, itemID: "att1" },
        ],
      };
    ...
    it('should return "broken" for an item with an attachment path that does not exist', async () => {
      const mockItem = {
        id: "item3",
        getAttachmentsObjects: async () => [
          { path: MOCK_INVALID_PATH, itemID: "att2" },
        ],
      };
      const status = await checker.getAttachmentStatus(mockItem);
    ...
    it('should return "error" if OS.File.exists throws an error', async () => {
      ...
      const mockItem = {
        id: "item5",
        getAttachmentsObjects: async () => [
          { path: MOCK_VALID_PATH, itemID: "att4" },
        ],
      };
      const status = await checker.getAttachmentStatus(mockItem);
      expect(status).toBe("error");
    ...
    describe("checkItems method", () => {
      it("should return an array of statuses for multiple items", async () => {
        const items = [
          ...
          {
            id: "itemC",
            getAttachmentsObjects: async () => [{ path: MOCK_INVALID_PATH }],
          },
        ];
...
global.expect = (actual) => ({
  ...
  toEqual: (expected) => {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expected)) {
      failures++;
      console.error(
        `AssertionError: Expected ${JSON.stringify(actual)} to be equal to ${JSON.stringify(expected)}`,
      );
    }
  },
  toContain: (expected) => {
...
async function runTests() {
  ...
  let statusValid = await checkerInstance.getAttachmentStatus({
    id: "manual1",
    getAttachmentsObjects: async () => [{ path: MOCK_VALID_PATH }],
  });
  console.log(`Status: ${statusValid}`);
  if (statusValid === "valid") assertions++;
  else {
    failures++;
    console.error("Manual valid item check FAILED");
  }

  console.log("\nManual check for missing attachment:");
  let statusMissing = await checkerInstance.getAttachmentStatus({
    id: "manual2",
    getAttachmentsObjects: async () => [],
  });
  console.log(`Status: ${statusMissing}`);
  if (statusMissing === "missing") assertions++;
  else {
    failures++;
    console.error("Manual missing item check FAILED");
  }

  console.log("\nManual check for broken link:");
  let statusBroken = await checkerInstance.getAttachmentStatus({
    id: "manual3",
    getAttachmentsObjects: async () => [{ path: MOCK_INVALID_PATH }],
  });
  console.log(`Status: ${statusBroken}`);
  if (statusBroken === "broken") assertions++;
  else {
    failures++;
    console.error("Manual broken link check FAILED");
  }
