      ...
      try {
        ...
        if (this.isArxivItem(item)) {
          ...
          if (publishedDOI) {
            ...
            this.log(
              `No published version found for item ${item.id}, converting to preprint`,
            );
            if (
  ...
  describe("AttachmentFinder - Enhanced arXiv Processing", () => {
    ...
    beforeEach(() => {
      ...
      testItem.setCreator(0, {
        firstName: "Ashish",
        lastName: "Vaswani",
        creatorTypeID: Zotero.CreatorTypes.getPrimaryIDForType(
          testItem.itemTypeID,
        ),
      });
    ...
    describe("Conference Paper Handling", () => {
      it("should identify conference venue from Semantic Scholar", async () => {
        const publishedInfo =
          await AttachmentFinder.findPublishedVersion(testItem);
        expect(publishedInfo).toBe(
          "VENUE:Neural Information Processing Systems|TITLE:Attention is All you Need",
        );
      });
      ...
      it("should convert to conference paper type for NIPS venue", async () => {
        const publishedInfo =
          "VENUE:Neural Information Processing Systems|TITLE:Attention is All you Need";
        await AttachmentFinder.updateItemAsPublishedVersion(
          testItem,
          publishedInfo,
        );

        expect(Zotero.ItemTypes.getName(testItem.itemTypeID)).toBe(
          "conferencePaper",
        );
        expect(testItem.getField("proceedingsTitle")).toBe(
      ...
      it("should download arXiv PDF for conference papers without DOI", async () => {
        const publishedInfo =
          "VENUE:Neural Information Processing Systems|TITLE:Attention is All you Need";
        await AttachmentFinder.downloadPublishedVersion(
          testItem,
          publishedInfo,
        );
    ...
    describe("Journal Article Handling", () => {
      it("should handle journal articles with DOI normally", async () => {
        ...
        AttachmentFinder.searchSemanticScholarForPublishedVersion = async () =>
          "10.1234/journal.example";

        const publishedInfo =
          await AttachmentFinder.findPublishedVersion(journalItem);
        ...
        expect(Zotero.ItemTypes.getName(journalItem.itemTypeID)).toBe(
          "journalArticle",
        );
    ...
    describe("Enhanced Search Strategies", () => {
      it("should accept proceedings-article type from CrossRef", async () => {
        ...
        const doi =
          await AttachmentFinder.searchCrossRefForPublishedVersion(testItem);
        expect(doi).toBe("10.5555/3295222.3295349");
      });

      it("should handle Semantic Scholar papers without DOI", async () => {
        const result =
          await AttachmentFinder.searchSemanticScholarForPublishedVersion(
            testItem,
          );
        expect(result).toBe(
          "VENUE:Neural Information Processing Systems|TITLE:Attention is All you Need",
      ...
      it("should calculate title similarity correctly", () => {
        const similarity1 = AttachmentFinder.titleSimilarity(
          "Attention Is All You Need",
          "Attention is All you Need",
        );
        expect(similarity1).toBeGreaterThan(0.95);

        const similarity2 = AttachmentFinder.titleSimilarity(
          "Attention Is All You Need",
    ...
    describe("Full Integration Tests", () => {
      ...
      it("should fallback to preprint conversion when no published version found", async () => {
        const unknownItem = new Zotero.Item(
        ...
        expect(Zotero.ItemTypes.getName(unknownItem.itemTypeID)).toBe(
          "preprint",
        );
...
async function runEnhancedArxivProcessorTests() {
  ...
  try {
    ...
    console.log(
      `✓ Updated item type: ${Zotero.ItemTypes.getName(testItem.itemTypeID)}`,
    );
    console.log(
      `✓ Proceedings title: ${testItem.getField("proceedingsTitle")}`,
    );

    // Test download
    await AttachmentFinder.downloadPublishedVersion(testItem, publishedInfo);
    console.log(
      `✓ Download tags: ${testItem.tags.map((t) => t.tag).join(", ")}`,
    );

    // Test 2: Full Integration
    console.log("\n2. Testing Full Integration...");
    const integrationItem = new Zotero.Item(
      Zotero.ItemTypes.getID("journalArticle"),
    );
    ...
    console.log(
      `✓ Final item type: ${Zotero.ItemTypes.getName(integrationItem.itemTypeID)}`,
    );
    console.log(
      `✓ Final tags: ${integrationItem.tags.map((t) => t.tag).join(", ")}`,
    );
    ...
    // Title similarity
    const similarity = AttachmentFinder.titleSimilarity(
      "Attention Is All You Need",
      "Attention is All you Need",
    );
    ...
    const enhancedExpectations = [
      {
        check: isVenueFormat,
        message: "Should detect venue format for conference papers",
      },
      { check: result.processed, message: "Should process arXiv items" },
      {
        check: result.foundPublished,
        message: "Should find published conference version",
      },
      {
        check:
          Zotero.ItemTypes.getName(integrationItem.itemTypeID) ===
          "conferencePaper",
        message: "Should convert to conference paper type",
      },
      {
        check:
          integrationItem.getField("proceedingsTitle") ===
          "Neural Information Processing Systems",
        message: "Should set proceedings title",
      },
      {
        check: integrationItem.hasTag("Conference PDF Downloaded"),
        message: "Should download conference PDF from arXiv",
      },
      {
        check: similarity > 0.95,
        message: "Should calculate high title similarity for similar titles",
      },
      {
        check: arxivId === "1706.03762",
        message: "Should extract arXiv ID correctly",
      },
    ];
